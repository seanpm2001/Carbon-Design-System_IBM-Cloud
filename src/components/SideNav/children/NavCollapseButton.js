...
const NavCollapseButton = ({
  ...
  return (
    <button
      aria-haspopup="true"
      aria-expanded={open}
      className={`pal--side-nav__collapse-button ${
        activeLabel ? "pal--side-nav__collapse-button--active" : ""
      }`}
      type="button"
      onClick={onClick}
    >
      ...
      {activeLabel && (
        <span className="pal--side-nav__collapse-button-active-item">
          {activeLabel}
        </span>
      )}
      <CollapseIcon className="pal--side-nav__collapse-button-icon" />
    </button>
  );
};
