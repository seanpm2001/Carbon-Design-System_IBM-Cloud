  ...
  return (
    <li
      className={classnames(
        "pal--side-nav__item",
        "pal--side-nav__item--with-menu",
        { "pal--side-nav__item--active": navMenuActive && !open },
        className
      )}
      {...rest}
    >
      <MenuButtonComponent
        ...
        aria-expanded={open}
        type="button"
        className="pal--side-nav__menu-button"
        id={id}
        onClick={toggleOpen}
      >
        {menuButtonLabel}
        <ChevronDown16 className="pal--side-nav__menu-icon" />
      </MenuButtonComponent>
      <ul className="pal--side-nav__menu">
        {items.map((item) => {
          const {
            onClick: onItemClick,
